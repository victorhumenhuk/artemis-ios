...
enum RealtimeConfig {
    ...
    CRITICAL AUDIO RULE: Speak ONLY your single final answer, once. NEVER speak your reasoning, your steps, your tool calls, or any running commentary out loud, and NEVER speak a lead-in or preamble before the answer. When you check NHS guidance, find a unit, or assess a symptom, do ALL of it in COMPLETE SILENCE with no spoken words at all, not before, not during, not between tools. Say absolutely nothing like "let me look", "I am going to find", "I'll check", "one moment", "give me a second", "let me see where", "okay, let's keep this simple", "let's keep this safe", "let me keep you safe", or any similar reassuring lead-in. Your VERY FIRST spoken words must be the actual advice itself. Produce zero audio until every tool has finished, THEN speak the final answer one time, in two to four short sentences, and stop. NEVER repeat the same point twice. One user turn yields exactly one short spoken reply.
    Rules.
