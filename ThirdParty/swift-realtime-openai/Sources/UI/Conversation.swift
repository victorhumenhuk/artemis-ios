...
public final class Conversation: @unchecked Sendable {
	...
	/// PATCH (Artemis): observe every raw server event for the developer console.
	public var onEvent: ((ServerEvent) -> Void)?

	/// PATCH (Artemis): the LOCAL mic audio WebRTC captures, for live on-device
	/// transcription on the exact audio the model hears.
	public var onLocalAudioBuffer: (@Sendable (AVAudioPCMBuffer) -> Void)? {
		get { client.onLocalAudioBuffer }
		set { client.onLocalAudioBuffer = newValue }
	}
	...
	public required init(debug: Bool = false, configuring sessionUpdateCallback: SessionUpdateCallback? = nil) throws {
		self.debug = debug
		client = try WebRTCConnector.create()   // PATCH: throwing, so a WebRTC failure is recoverable, not a crash
...
private extension Conversation {
	func handleEvent(_ event: ServerEvent) throws {
		...
		onEvent?(event)   // PATCH (Artemis): forward every raw event to observers
		...
		switch event {
			...
			case let .conversationItemCreated(_, item, _):
				entries.append(item)
			// PATCH (Artemis): GA delivers the USER input_audio item via
			// conversation.item.added (NOT .created). Without appending it here the
			// later input_audio_transcription.delta/.completed never match an entry
			// (updateEvent finds nothing) and her spoken words never render.
			case let .conversationItemAdded(_, item, _):
				if !entries.contains(where: { $0.id == item.id }) { entries.append(item) }
			// PATCH (Artemis): GA (gpt-realtime-2) delivers the model's output
			// items (assistant messages and function calls) via
			// response.output_item.added, which upstream does not append. Without
			// this, transcripts and tool calls never surface. See README "SDK patch".
			...
			case let .conversationItemInputAudioTranscriptionCompleted(_, itemId, contentIndex, transcript, _, _):
				updateEvent(id: itemId) { message in
					while message.content.count <= contentIndex {
						message.content.append(.inputAudio(.init(audio: nil as AudioData?, transcript: "")))
					}
			...
			// PATCH (Artemis): apply the live input-transcription delta so her own
			// words appear AS she speaks, not only when transcription completes.
			case let .conversationItemInputAudioTranscriptionDelta(_, itemId, contentIndex, delta, _):
				updateEvent(id: itemId) { message in
					// The user audio item is created with an EMPTY content array, so grow
					// it to the delta's index first, otherwise every delta is dropped and
					// the live transcript never appears (the recurring bug).
					while message.content.count <= contentIndex {
						message.content.append(.inputAudio(.init(audio: nil as AudioData?, transcript: "")))
					}
					guard case let .inputAudio(audio) = message.content[contentIndex] else { return }
					message.content[contentIndex] = .inputAudio(.init(audio: audio.audio, transcript: (audio.transcript ?? "") + delta))
				}
			...
			case let .responseContentPartAdded(_, _, itemId, _, contentIndex, part):
				updateEvent(id: itemId) { message in
					guard contentIndex <= message.content.count else { return }   // PATCH: bounds
			...
			case let .responseOutputItemDone(_, _, _, item):
				// PATCH (Artemis): upsert the finalised item (message OR function
				// call) so tool calls reach .completed and transcripts finalise.
